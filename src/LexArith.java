...
public abstract class LexArith extends IO
{
...

	{
		...
		while ( a != -1 ) // do the body if "a" is not end-of-stream
		{
			...
			if ( nextSt == State.Keyword_default || nextSt == State.Keyword_case || nextSt == State.Keyword_do ||
					nextSt == State.Keyword_else || nextSt == State.Keyword_false || nextSt == State.Keyword_for ||
					nextSt == State.Keyword_if || nextSt == State.Keyword_print || nextSt == State.Keyword_switch ||
					nextSt == State.Keyword_true || nextSt == State.Keyword_while) {
				state = nextSt;
				return 1; // keyword extracted
			}
...
//				return State.UNDEF;
	...
	{		
		...
		while ( a != -1 ) // while "a" is not end-of-stream
		{
			...
			if ( i == 1 )
				displayln( t.trim() + "   : " + state.toString() );
			else if ( i == 0 )
				displayln( t+" : Lexical Error, invalid token");
		} 
